...
import { generateId, showToast, customAlert, customConfirm, closeModal, validatePhoneNumber, getDefaultAcademicYearAndSemester } from '../utils/helpers.js';
...
export function renderMasterData() {
    ...
    document.getElementById('tbody-teachers').innerHTML = filteredTeachers.map(t => {
        const subs = t.subjects ? t.subjects.map(sid => { const s = AppState.allSubjects.find(x=>x.id===sid); return s?s.name:''; }).filter(Boolean).join(', ') : '-';
        return `<tr>
    ...
    let filteredClasses = [...AppState.allClasses];
    if (searchClass) {
    ...
    document.getElementById('tbody-classes').innerHTML = sortedClasses.map(c => {
        const advs = c.advisors ? c.advisors.map(tid => { const t = AppState.allTeachers.find(x=>x.id===tid); return t?t.firstName:''; }).filter(Boolean).join(', ') : '-';
        const subs = c.subjects ? c.subjects.map(sid => { const s = AppState.allSubjects.find(x=>x.id===sid); return s?s.name:''; }).filter(Boolean).join(', ') : '-';
        return `<tr>
...
export function deleteMaster(type, id) {
    ...
    customConfirm('ยืนยันการลบข้อมูล', 'คุณต้องการลบข้อมูลนี้ใช่หรือไม่? ข้อมูลที่ลบจะไม่สามารถกู้คืนได้', async () => {
        if(type==='subject') { AppState.allSubjects = AppState.allSubjects.filter(x=>x.id!==id); await saveToDB(DB_KEYS.SUBJECTS, AppState.allSubjects, 'saveSubjects'); }
        if(type==='teacher') { AppState.allTeachers = AppState.allTeachers.filter(x=>x.id!==id); await saveToDB(DB_KEYS.TEACHERS, AppState.allTeachers, 'saveTeachers'); }
        if(type==='class') { AppState.allClasses = AppState.allClasses.filter(x=>x.id!==id); await saveToDB(DB_KEYS.CLASSES, AppState.allClasses, 'saveClasses'); }
        renderMasterData(); 
...
// Subject CRUD
export function openSubjectModal() { document.getElementById('sub-id').value=''; document.getElementById('sub-code').value=''; document.getElementById('sub-name').value=''; document.getElementById('sub-credit').value=''; document.getElementById('subject-modal').classList.add('show'); }
export function editSubject(id) {
    const s = AppState.allSubjects.find(x=>x.id===id); if(!s) return;
    document.getElementById('sub-id').value=s.id; document.getElementById('sub-code').value=s.code; document.getElementById('sub-name').value=s.name; document.getElementById('sub-credit').value=s.credit;
    document.getElementById('subject-modal').classList.add('show');
}
export async function saveSubject() {
    const obj = { id: document.getElementById('sub-id').value||generateId(), code: document.getElementById('sub-code').value, name: document.getElementById('sub-name').value, credit: document.getElementById('sub-credit').value };
    if(!obj.code || !obj.name) return customAlert('กรุณากรอกรหัสและชื่อวิชาให้ครบถ้วน');
    const idx = AppState.allSubjects.findIndex(x=>x.id===obj.id); if(idx>-1) AppState.allSubjects[idx]=obj; else AppState.allSubjects.push(obj);
...
export function openTeacherModal() {
    ...
    document.getElementById('t-subjects-container').innerHTML = AppState.allSubjects.map(s => `<label class="checkbox-container">${s.code} - ${s.name}<input type="checkbox" value="${s.id}" class="t-sub-cb"><span class="checkmark"></span></label>`).join('');
    document.getElementById('teacher-modal').classList.add('show');
}
export function editTeacher(id) {
    if (AppState.currentUser && AppState.currentUser.role !== 'admin') return customAlert('เฉพาะผู้ดูแลระบบเท่านั้นที่จัดการข้อมูลนี้ได้');
    const t = AppState.allTeachers.find(x=>x.id===id); if(!t) return;
    ...
    document.getElementById('t-subjects-container').innerHTML = AppState.allSubjects.map(s => `<label class="checkbox-container">${s.code} - ${s.name}<input type="checkbox" value="${s.id}" class="t-sub-cb" ${t.subjects && t.subjects.includes(s.id)?'checked':''}><span class="checkmark"></span></label>`).join('');
    document.getElementById('teacher-modal').classList.add('show');
}
export async function saveTeacher() {
    ...
    const obj = { 
        id: document.getElementById('t-id').value||generateId(), 
        title: document.getElementById('t-title').value, 
        firstName: document.getElementById('t-fname').value, 
        lastName: document.getElementById('t-lname').value, 
    ...
    if(!obj.email) return customAlert('กรุณากรอกอีเมลสำหรับเข้าสู่ระบบ');
    if(!obj.firstName || !obj.lastName) return customAlert('กรุณากรอกชื่อและนามสกุลให้ครบถ้วน');
    if(obj.phone && !validatePhoneNumber(obj.phone)) return customAlert('เบอร์โทรศัพท์ครูไม่ถูกต้อง');
    
    const existEmail = AppState.allTeachers.find(x => x.email === obj.email && x.id !== obj.id);
    if(existEmail) return customAlert('อีเมลนี้ถูกใช้งานไปแล้ว กรุณาใช้อีเมลอื่น');

    const idx = AppState.allTeachers.findIndex(x=>x.id===obj.id); if(idx>-1) AppState.allTeachers[idx]=obj; else AppState.allTeachers.push(obj);
...
export function openClassModal() {
    ...
    document.getElementById('c-advisors-container').innerHTML = AppState.allTeachers.map(t => `<label class="checkbox-container">${t.firstName} ${t.lastName}<input type="checkbox" value="${t.id}" class="c-adv-cb"><span class="checkmark"></span></label>`).join('');
    document.getElementById('c-subjects-container').innerHTML = AppState.allSubjects.map(s => `<label class="checkbox-container">${s.code} - ${s.name}<input type="checkbox" value="${s.id}" class="c-sub-cb"><span class="checkmark"></span></label>`).join('');
    document.getElementById('class-modal').classList.add('show');
}
export function editClass(id) {
    const c = AppState.allClasses.find(x=>x.id===id); if(!c) return;
    ...
    document.getElementById('c-advisors-container').innerHTML = AppState.allTeachers.map(t => `<label class="checkbox-container">${t.firstName} ${t.lastName}<input type="checkbox" value="${t.id}" class="c-adv-cb" ${c.advisors && c.advisors.includes(t.id)?'checked':''}><span class="checkmark"></span></label>`).join('');
    document.getElementById('c-subjects-container').innerHTML = AppState.allSubjects.map(s => `<label class="checkbox-container">${s.code} - ${s.name}<input type="checkbox" value="${s.id}" class="c-sub-cb" ${c.subjects && c.subjects.includes(s.id)?'checked':''}><span class="checkmark"></span></label>`).join('');
    document.getElementById('class-modal').classList.add('show');
}
export async function saveClassRoom() {
    ...
    const obj = { id: document.getElementById('c-id').value||generateId(), year: document.getElementById('c-year').value, semester: document.getElementById('c-sem').value, className: document.getElementById('c-name').value, advisors: advs, subjects: subs };
    if(!obj.className) return customAlert('กรุณากรอกชื่อชั้นเรียน');
    const idx = AppState.allClasses.findIndex(x=>x.id===obj.id); if(idx>-1) AppState.allClasses[idx]=obj; else AppState.allClasses.push(obj);
